...
| read_appia_messages | 读取 Appia 未读消息或指定房间的最近消息 | 无 |
...
7. 联动 Appia：设置 → Appia 连接，填入服务器地址、Rocket.Chat 用户 ID 和访问令牌（Appia 网页端 个人设置 → 个人访问令牌 创建；`X-User-Id` 填令牌页面显示的 User ID，不是登录用户名），然后对 agent 说「给 @某某 发条 Appia 消息说……」「我有什么未读消息」「看看 #项目群 最近在聊什么」

另外，read_screen / screen_action 是通用屏幕工具，无需任何适配即可直接操作 Appia 应用界面（打开聊天、读屏、点击发送），适合 API 令牌不便获取时的备选方案；配合「主动提醒」模块还能实现「每天 9 点读取未读并汇总发到通知栏」这类自动化。
